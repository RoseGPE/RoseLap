...
class Segment(object):
  def __init__(self,x1,y1,x2,y2,x3,y3,sector):
    ...
    self.length_m = math.sqrt((self.x_m-self.x)**2 + (self.y_m-self.y)**2);
    self.length_p = math.sqrt((self.x_p-self.x)**2 + (self.y_p-self.y)**2);
    self.curvature = 0; #compute me pls
    self.sector = sector;
...
def plot_segments(segments):
  ...
  for idx,sector in enumerate(sectors):
    ax.scatter(sector[:,0], sector[:,1], label=idx)
...
if __name__ == '__main__':
  ...
  #[print(x) for x in dxf_geometry]
  #print(connectivity)

  points = pointify_dxf(dxf_geometry,connectivity,5)
  #[print(x) for x in points]

  segs = seg_points(points)

  #[print(x.x, x.y, x.length_p+x.length_m) for x in segs]
